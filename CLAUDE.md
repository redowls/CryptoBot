...
Sessions 1–9 of 10 complete (data client, DB schema/repo, historical ingest, Donchian strategy, vectorbt backtest, Alpaca executor + APScheduler wiring in main.py, RiskManager + 1-min stop monitor + 1-hr equity snapshot job, real Telegram bridge + rate-limited ERROR mirror + 00:00 UTC daily summary + top-level crash handler). Session 10 (systemd deploy) is **drafted but not yet acceptance-tested** — `deploy/cryptobot.service`, `deploy/install.sh`, `deploy/update.sh`, and `scripts/healthcheck.py` exist and the README has VPS deployment + log inspection sections, but the 7-day VPS uptime test from initial.md §6 still needs a live Contabo box. [initial.md](initial.md) is the 450-line build plan and source of truth for design decisions — read it before implementation work; each session's verbatim prompt and acceptance test lives in §6. The repo has a single initial commit on `main` covering Sessions 1–9 — per-session deliverables are not separated in git history, so for "what did session N add" consult initial.md §6 rather than `git log`.
...
Session 10 scaffolding: [deploy/](deploy/) (`cryptobot.service` systemd unit, `install.sh` idempotent provisioner, `update.sh` for `git pull` + restart) and [scripts/healthcheck.py](scripts/healthcheck.py) (DB `SELECT 1` + Alpaca `get_account_info` + last-`signals.ts` age probe; exits non-zero on failure for cron / uptime monitors). `logs/` exists but is gitignored (rotating file logs land here at runtime). `backtest/results/` is created on first plot save and holds the per-symbol equity-curve PNGs.
